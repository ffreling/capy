...
pub fn Atom(comptime T: type) type {
    return struct {
        ...
        fn extendedSet(self: *Self, value: T, comptime options: ExtendedSetOptions) void {
            ...
            if ((if (@hasDecl(atomicValue(bool), "cmpxchgStrong")) self.bindLock.cmpxchgStrong(false, true, .seq_cst, .seq_cst) else self.bindLock.cmpxchg(true, false, true, .seq_cst, .SeqCst) // support zig 0.11 as well as current master
            ) == null) {
